...
public class VistaVincularTurno extends JFrame implements IGUI {
	
	private static final long serialVersionUID = 1L;
	
	public VistaVincularTurno() {
		
		setDefaultCloseOperation(EXIT_ON_CLOSE);
		setTitle("VINCULAR EMPLEADO A TURNO");
		...
		JButton show = new JButton("VINCULAR");
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (id.getText().length() != 0 && text.getText().length() != 0) {
						...
						Controlador.getInstance().accion(Evento.VINCULAR_TURNO, tTrabaja);
	...
	public void actualizar(int evento, Object datos) {
		setVisible(false);
			if(evento == Evento.RES_VINCULAR_TURNO_OK) {
				JOptionPane.showMessageDialog(this, "El empleado se ha vinculado correctamente " );
			} 
			else {
				JOptionPane.showMessageDialog(this, "El Empleado no se ha podido vincular");
			}
